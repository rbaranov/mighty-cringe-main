...
Откройте `http://localhost:5173`, начните тренировку с жимом лёжа и один раз выберите голос в
«Пояснить». Закройте панель и снова нажмите «Пояснить»: запись должна начаться сразу. После
остановки кратко появится строка «Распознаю и разбираю…», затем панель сама получит фиксированную
тестовую фразу
`Можешь заменить жим лежа на жим лежа на наклонной скамье` и покажет подтверждение замены без
кнопки «Разобрать фразу». Нажмите «Исправить фразу», проверьте доступность редактирования, повторите
запись и нажмите «Выполнить». В настройках запись должна воспроизводиться. После `Ctrl+C` тестовое
серверное аудио исчезает.
