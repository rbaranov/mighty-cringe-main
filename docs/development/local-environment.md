...
в IndexedDB, отправку, обновление статуса и локальное воспроизведение. Через 1,2 секунды сервер
возвращает фиксированную тестовую расшифровку, не анализируя и никуда не отправляя аудио.
...
После остановки кратко появится строка «В очереди распознавания», затем панель сама получит текст
`жим штанги лежа 40 на 10 rir 2` и передаст его в обычный разборщик. В настройках запись должна
воспроизводиться. После `Ctrl+C` тестовое серверное аудио исчезает.
